...
async function refreshCache(force = false) {
  ...
  try {
    ...
    return { cache, refreshed: true, stale: false, warning: null };
  } catch (err) {
    if (existing && Array.isArray(existing.recipes) && existing.recipes.length > 0) {
      return {
        cache: existing,
        refreshed: false,
        stale: true,
...
async function handle(req, res) {
  ...
  if (path === '/api/refresh') {
    try {
      ...
      const { cache, refreshed, stale = false, warning = null } = await refreshCache(force);
      return json(res, 200, {
        ok: true,
        refreshed,
        stale,
  ...
  if (path === '/api/recipes') {
    try {
      const { cache, stale = false, warning = null } = await refreshCache(false);
      return json(res, 200, {
        ok: true,
        stale,
        warning,
  ...
  if (path === '/api/random') {
    try {
      ...
      return json(res, 200, { ok: true, stale, warning, recipe: cache.recipes[idx] });
    } catch (err) {
      return json(res, 500, { ok: false, error: err.message });
    }
  }
