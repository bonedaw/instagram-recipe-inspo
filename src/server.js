    ...
    if (path === '/api/random') {
      try {
        ...
        if (!cache.recipes.length) {
          return json(res, 404, { ok: false, error: 'No recipes found' });
        }
    ...
    if (path === '/api/search' || path === '/api/suggestions') {
      try {
        ...
        const ingredients = parseCsv(
          reqUrl.searchParams.get('ingredients') ||
            reqUrl.searchParams.get('includeTags')
        );
        const excludeIngredients = parseCsv(
          reqUrl.searchParams.get('excludeIngredients') ||
            reqUrl.searchParams.get('excludeTags')
        );
